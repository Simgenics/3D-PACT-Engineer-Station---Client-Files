...
public class NewBundleWindow : SolitaryWindow
{
	bool gotCompanies;
	...
	private void OnGUI()
	{
		...
		using (var areaScope = new GUILayout.AreaScope(new Rect(650, 400, 200, 30)))
		{
			if (selectedCompany != null && selectedFacility != null)
			{
				if (GUILayout.Button("Next", GUILayout.Width(200), GUILayout.Height(30)))
				{
					...
					{
						var wnd = GetWindow<UploadNewBundle>(false, "New");
	...
	void BeforeClose(bool failed)
	{
		...
		if (!failed)
		{
			if (EditorPrefs.GetString("SelectedBundleStatus") == "Locked")
				Debug.Log($"<color={"#ffcc66"}>{"Uploading New step 1 of 3: Building Bundle"}</color>\n");
			else
				Debug.Log($"<color={"#ffcc66"}>{"Uploading New step 1 of 3: Building Package"}</color>\n");
		}
#endif
		Close();
	}
	...
	void ShowCompanyDetails()
	{
		...
		GUILayout.Space(20);
		string desc = "";
		if (!string.IsNullOrEmpty(selectedCompany.description))
			desc = selectedCompany.description.Replace("\n", "");
		desc = desc.Replace("\r", "");
		desc = desc.Replace("\r\n", "");
